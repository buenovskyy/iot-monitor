...
Funcionalidades:
  - API REST via HTTP (Flask)
  - Cadastro e autenticação de usuários
  - Persistência de dados (SQLite)
  - Registro de logs
  - Medição de RTT e throughput
  - Tratamento de erros
  - Comunicação em tempo real (WebSocket via SocketIO)
  - Recepção de dados via UDP
"""
...
#database
def get_db():
    """Obtém conexão com o banco SQLite (uma por request)."""
    if 'db' not in g:
...
def autenticacao_requerida(f):
    """Decorator para proteger rotas com autenticação via token."""
    @wraps(f)
...
@app.before_request
def antes_request():
    """Registra o início do processamento para calcular tempo de resposta."""
    g.inicio = time.time()

@app.after_request
def depois_request(response):
    """Registra log de rede com métricas de cada requisição."""
    if hasattr(g, 'inicio'):
...
@app.route('/')
def index():
    """Serve o dashboard web (será criado na Parte 4)."""
...
def criar_sensor():
    ...
    Body JSON: {"nome": "...", "tipo": "temperatura", "localizacao": "Sala 1"}
    """
    dados = request.get_json()
...
def verificar_alerta(sensor_id, tipo, valor):
    """Verifica se o valor excede os limites e gera alerta automático."""
...
def enviar_leitura():
    """
    Envia uma leitura de sensor via HTTP.
    Body JSON: {"sensor_id": 1, "valor": 25.3, "unidade": "°C"}
    """
    dados = request.get_json()
...
def consultar_leituras(sensor_id):
    """
    Consulta leituras de um sensor.
    Query params: ?limite=100&desde=2026-01-01
    """
