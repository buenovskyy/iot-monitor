#!/usr/bin/env python3
"""
benchmark.py - Benchmark COMPLETO do IoT Monitor
Mede RTT, throughput e tamanho de pacotes para HTTP, UDP e WebSocket.

Uso:
  1. Inicie o servidor: python run_server.py
  2. Em outro terminal: python benchmark.py

O script salva os resultados em benchmark_results.json
"""
...
def setup():
    """Registra usuário e cria sensores para os testes."""
    sep("SETUP")
...
def benchmark_rtt_websocket(n=30):
    ...
    # Fallback: se o servidor não tiver 'pong_benchmark',
    # usamos o evento padrão 'nova_leitura' como eco
    @sio.on('nova_leitura')
    def on_leitura(data):
    ...
    for i in range(n):
        ...
        # Tenta emitir ping_benchmark primeiro, senão usa simular_leitura
        try:
            sio.emit('ping_benchmark', payload)
        except:
            sio.emit('simular_leitura', payload)

        # Espera resposta (timeout 2s)
    ...
    if not rtts:
        ...
        # Fallback: medir RTT via HTTP com conexão persistente (simula WS)
...
# ============================================================
# 7. ESCALABILIDADE (múltiplas requisições)
# ============================================================
